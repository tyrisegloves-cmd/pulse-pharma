...
-- IMPORTANT: drop EVERY existing policy, not just the two we expect. If a
-- stray policy was added manually in the Dashboard (e.g. one whose USING
-- expression calls a function that itself reads `profiles`), it causes
-- "infinite recursion detected in policy for relation profiles" (SQLSTATE
-- 42P17). That recursion aborts the signup trigger, which makes auth.users
-- insert fail, surfacing as HTTP 500 "Database error saving new user" on the
-- Sign Up form. Purging all policies before recreating guarantees a clean,
-- non-recursive state no matter what was deployed before.
do $$
declare
  p record;
begin
...
-- `auth.uid()` is a stable built-in returning the current JWT sub; it does
-- NOT query `profiles`, so this policy cannot recurse.
